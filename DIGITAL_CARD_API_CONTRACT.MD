...
4. Generates a one-time login mechanism (magic link, OTP, or agreed method
   — see Schema Questions Q3)
...
`token_type` tells mobile how to consume `login_token`:
- `"magic_link"` → pass to `supabase.auth.getSessionFromUrl(login_token)`
- `"otp"` → pass to `supabase.auth.verifyOtp({ token: login_token, type: 'email', email })`
...
{
  "recovery_session_id": "opaque-token-from-step-1",
  "card_file_storage_path": "recovery-files/{creator_id}/{request_id}/card.png",
  "selfie_video_storage_path": "recovery-files/{creator_id}/{request_id}/selfie.mp4"
}
```
...
```json
{
  ...
  "storage_path": "recovery-files/{creator_id}/{request_id}/card.png",
...
Mobile cannot go live until these are available in order:

1. ✅ Schema questions answered (see DIGITAL_CARD_SCHEMA_QUESTIONS.md)
2. 🔲 New tables created with correct RLS
3. 🔲 Endpoint 3 (generation status) — mobile needs this to build the UI
4. 🔲 Endpoint 1 (generate token) — blocks card QR payload change
5. 🔲 Endpoint 2 (store fingerprint) — must be available alongside Endpoint 1
6. 🔲 Endpoint 4 (verify card) — blocks recovery flow
7. 🔲 Endpoint 7 (upload URL) — blocks manual recovery
8. 🔲 Endpoint 5 (complete recovery) + Persona webhook handling
9. 🔲 Endpoint 6 (submit manual) — can be last, unverified path
