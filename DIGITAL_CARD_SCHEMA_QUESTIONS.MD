...
## Summary — What we need before mobile writes code

| # | Blocker | Who unblocks it |
|---|---------|----------------|
| Q1 | Persona verified column name | Web team confirms |
| Q2 | Where generation counters live + reset ownership | Web team decides + builds |
| Q3 | One-time login token mechanism | Web team decides + builds |
| Q4 | Recovery file storage bucket + path | Web team creates bucket |
| Q5 | `professional_headline` column confirmation | Web team confirms |
| Q6 | `card_photo_url` column confirmation | Web team confirms |
| Q7 | RLS policy confirmation on new tables | Web team sets RLS |
| Q8 | Admin dashboard ownership | Web team confirms |
| Q9 | Rate limit mechanism + response format | Web team builds + documents |

Mobile can begin building the UI skeletons and TypeScript types in parallel,
but no API calls can be wired until Q1, Q2, Q3, and Q7 are resolved.
