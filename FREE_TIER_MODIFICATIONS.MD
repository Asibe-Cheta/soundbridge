...
All storage quota logic on the mobile side reads from `StorageQuotaService` which already has the correct 250MB value. The **web app** was updated to match: copy and tier messaging now use 250MB everywhere, and backend constants (`grace-period-service`, `api/user/storage-status`) were set to 250MB so web and mobile enforce the same limit.
