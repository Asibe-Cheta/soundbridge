...
type ProviderPayoutHistoryRow = {
  ...
  amount?: number;
  currency?: string;
  /** Fincra / provider reference (same value as stripe_transfer_id in DB). */
...
function payoutRef(h: ProviderPayoutHistoryRow): string {
  return (h.provider_transfer_id || h.stripe_transfer_id || h.id) as string;
}
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={pending.length === 0 ? 'No pending payouts' : 'Send all pending Fincra (NGN/GHS/KES) payout requests'}
          >
        ...
        <div className={`mb-6 rounded-lg border p-4 ${cardClass}`}>
          ...
          {lastBatchResult.processed > 0 && (
            <p className={`text-sm ${mutedClass} mb-3`}>
              ...
              {lastBatchResult.processed} payout{lastBatchResult.processed !== 1 ? 's' : ''} submitted via Fincra.
            </p>
          )}
          ...
          {lastBatchResult.submissionErrors.length > 0 && (
            <div>
              <p className={`text-sm font-medium ${textClass} mb-2`}>Failed (Fincra / API error)</p>
              <ul className="list-disc list-inside space-y-2 text-sm">
                {lastBatchResult.submissionErrors.map((se, idx) => {
                  const row = pending.find((p) => p.id === se.payout_request_id);
                  const name = row?.creator_name ?? row?.creator_email ?? se.payout_request_id.slice(0, 8) + '…';
                  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className={`rounded-lg border p-4 ${cardClass}`}>
          <div className={`text-sm ${mutedClass}`}>Processing queue</div>
          <div className="text-3xl font-semibold text-white">{processing.length}</div>
      ...
      <div className={`rounded-lg border overflow-hidden mb-6 ${cardClass}`}>
        <div className="p-4 border-b flex items-center justify-between">
          ...
          <span className={`text-sm ${mutedClass}`}>Approve via Fincra, or mark paid manually if you sent funds outside the platform rail</span>
        </div>
                    ...
                    <td className="p-3 text-right">
                      <div className="flex flex-wrap justify-end gap-2">
                        ...
                        <button
                          ...
                          title="Use when funds were sent manually (bank transfer, etc.) — no Fincra API call"
                        >
      ...
      <div className={`rounded-lg border overflow-hidden mb-6 ${cardClass}`}>
        <div className="p-4 border-b flex items-center justify-between">
          ...
          <span className={`text-sm ${mutedClass}`}>Provider or bank reported failure — Retry re-queues as pending</span>
        </div>
        {loading ? (
          <div className="p-8 text-center text-gray-500">Loading…</div>
