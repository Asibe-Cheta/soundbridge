  ...
  async toggleLike(userId: string, request: CreateLikeRequest): Promise<{ data: Like | null; error: any }> {
    try {
      ...
      if (existingLike) {
        ...
        // Event like: in-app notification only (track likes: POST /api/social/likes calls notifyTrackLiked)
          ...
          if (creatorId && creatorId !== userId) {
            try {
              ...
              if (profileError || !creatorProfile) {
                console.error('❌ Event like notification skipped (profile):', profileError);
              } else {
              ...
              console.error('❌ Error creating event like notification:', notificationError);
            }
          }
        }
