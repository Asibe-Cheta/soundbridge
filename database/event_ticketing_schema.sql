-- Event Ticketing System Schema for SoundBridge
...
CREATE INDEX idx_event_tickets_active ON event_tickets(is_active) WHERE is_active = true;

-- ============================================
-- PART 2: TICKET PURCHASES
-- ============================================
...
CREATE INDEX idx_ticket_purchases_ticket_code ON ticket_purchases(ticket_code);
CREATE INDEX idx_ticket_purchases_stripe_intent ON ticket_purchases(stripe_payment_intent_id);

-- ============================================
-- PART 3: BUNDLES (Album + Ticket Packages)
-- ============================================
...
CREATE INDEX idx_event_bundles_event ON event_bundles(event_id);
CREATE INDEX idx_event_bundles_active ON event_bundles(is_active) WHERE is_active = true;
...
CREATE INDEX idx_bundle_purchases_user ON bundle_purchases(user_id);
CREATE INDEX idx_bundle_purchases_bundle ON bundle_purchases(bundle_id);
...
CREATE VIEW user_friends_attending_events AS
SELECT DISTINCT
  e.id AS event_id,
...
CREATE INDEX idx_listening_history_artist ON user_listening_history(artist_id);
CREATE INDEX idx_listening_history_genre ON user_listening_history(genre);
...
-- Update creator revenue when tickets are sold
CREATE OR REPLACE FUNCTION record_ticket_sale_revenue()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.payment_status = 'succeeded' AND OLD.payment_status != 'succeeded' THEN
    ...
    DECLARE
      creator_id UUID;
    BEGIN
      SELECT e.creator_id INTO creator_id
      FROM events e
      WHERE e.id = NEW.event_id;
      
      -- Update creator revenue
      INSERT INTO creator_revenue (user_id, total_earned, pending_balance, available_balance)
      VALUES (creator_id, NEW.promoter_revenue, NEW.promoter_revenue, 0)
      ON CONFLICT (user_id) 
      DO UPDATE SET
        total_earned = creator_revenue.total_earned + NEW.promoter_revenue,
        pending_balance = creator_revenue.pending_balance + NEW.promoter_revenue,
        updated_at = NOW();
      ...
      INSERT INTO revenue_transactions (
        ...
        creator_id,
        'ticket_sale',
        NEW.promoter_revenue,
        'Ticket sale for event',
...
CREATE TRIGGER trigger_record_ticket_revenue
  AFTER UPDATE ON ticket_purchases
  FOR EACH ROW
  EXECUTE FUNCTION record_ticket_sale_revenue();
...
CREATE INDEX idx_ticket_purchases_user_status ON ticket_purchases(user_id, status);
CREATE INDEX idx_ticket_purchases_event_status ON ticket_purchases(event_id, payment_status);
CREATE INDEX idx_event_bundles_event_active ON event_bundles(event_id, is_active);
CREATE INDEX idx_listening_history_user_artist ON user_listening_history(user_id, artist_id);
