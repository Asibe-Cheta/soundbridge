-- Revenue Sharing & Monetization Schema for SoundBridge (Fixed Version)
...
DROP TRIGGER IF EXISTS update_paid_content_updated_at ON paid_content;
DROP TRIGGER IF EXISTS update_creator_tips_updated_at ON creator_tips;
DROP TRIGGER IF EXISTS update_creator_subscriptions_updated_at ON creator_subscriptions;
DROP TRIGGER IF EXISTS update_creator_subscription_tiers_updated_at ON creator_subscription_tiers;
...
-- Create triggers for updated_at (recreated after dropping)
...
CREATE TRIGGER update_paid_content_updated_at 
  BEFORE UPDATE ON paid_content
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_creator_tips_updated_at 
  BEFORE UPDATE ON creator_tips
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_creator_subscription_tiers_updated_at 
  BEFORE UPDATE ON creator_subscription_tiers
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Insert default revenue records for existing users (only if they don't exist)
