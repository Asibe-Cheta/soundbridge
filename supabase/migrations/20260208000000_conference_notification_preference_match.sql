-- Conference notification preference match
-- Conference events are stored as category 'Other'. Users who selected "Conference" or
-- "Conferences & Seminars" in preferences have that string in preferred_event_genres,
-- so they were never matched. This migration makes 'Other' events also match those preferences.
  ...
  WHERE
    ...
    AND (
      ...
      OR (
        ...
        AND (
          'Conference' = ANY(np.preferred_event_genres)
          OR 'Conferences & Seminars' = ANY(np.preferred_event_genres)
        )
      )
    )
    AND (
